...
LAW = {
    ...
    "agent": (
        ...
        "work is driven by a *.plan.md, open it in the host's plan editor by running the "
        "command the capability note in your system prompt teaches, with the plan's absolute "
        "path, as you take the plan up; a session with no capability note has nothing to run."
    ),
    "agent-loop": (
        ...
        "direction\n"
        "• plan surfacing — when your work is driven by a *.plan.md, surface it in the host's "
        "plan editor: the capability note in your system prompt names the exact open command — "
        "run it with the plan's ABSOLUTE path as you take the plan up, and again when you switch "
        "plans; the printed acknowledgment is authoritative — opening the tab is the host's job, "
        "CONTINUE the turn; a session with no capability note (running outside CCVI) has nothing "
        "to run — the duty is inert there\n"
