...
BBP ends at the push. **Never install the built plugin.** Refreshing this machine's
live copy under `~/.ccvi/` is a MANUAL step the user performs themselves - do not
unzip into it, do not offer to, and do not treat a stale installed version as a
problem to fix. Self-install proved problematic; manual install is the only route.
